# Helper class for managing multiple sensors, can probably be put into ros_dataparser.py
...
def tf_to_file(tf_data):
    transform = ros_numpy.numpify(tf_data.transform)
    rot_matrix = np.array(euler_matrix(0, np.pi, np.pi))
    return (transform @ rot_matrix).tolist()


def check_type_match(topic_name: str, cam_type: CameraType):
    return False
    if 'rect' in topic_name and cam_type != CameraType.PERSPECTIVE:
        CONSOLE.print(
...
class Sensor():
    ...
    def setup(self, dataset):
        ...
        # Start by figuring out what type of messages we are dealing with
        # TODO: Use image transport/arg for this?
    ...
    def info_cb(self, info: CameraInfo = None):
        ...
        if self.use_preset_D:
            ...
            # TODO: Update these based with new nerfstudio names
            ...
            # Check if rectified image with non-rectified camera info
    ...
    def save_depth(self, depth: Union[Image, CompressedImage]):
        ...
        # CONSOLE.print(f"{self.name} got depth {depth.header.seq} for {self.dataset.mode} dataset, {self.dataset.current_idx} / {self.dataset.num_images}")
    ...
    def save_img(self, image, header):
    ...
    def save_to_file_synced(self, rgb: Union[Image, CompressedImage], depth: Union[Image, CompressedImage]):
        ...
        cv2.imwrite(
            f"{self.dataset.output_path}/depth/{self.name}_{depth.header.seq:05}.png", depth_image)
    ...
    def save_checker(self) -> bool:
        ...
        # If we the service call says to stop, don't save images
