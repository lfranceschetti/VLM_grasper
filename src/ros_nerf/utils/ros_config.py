...
class ROSConfig:
    ...
    num_lidar_start: Suppress[int] = 0
    """Number of lidar pts to use for splatting initialization (-1 means all until num_images reached)"""
    ...
    lidars: Suppress[list] = None
    """List of lidars to use, populated at runtime."""
    ...
    def update(self) -> str:
        ...
        # Set lidars to empty list if not specified
        if self.lidars == None:
            self.lidars = []
    ...
    def check_config(self, config: dict) -> None:
        ...
        if "lidars" in config and ("num_lidar_start" not in config or config["num_lidar_start"] == 0):
            raise ValueError("Number of lidar points must be specified in config file")
