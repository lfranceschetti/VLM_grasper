...
#But changed to use pytorch instead of tensorflow
...
class PandaGripper(object):
    ...
    def __init__(self, q=None, num_contact_points_per_finger=10, root_folder=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))):
        ...
        self.q = q
        fn_base = os.path.join(root_folder, 'data_robot/gripper_models/panda_gripper/hand.stl')
        fn_finger = os.path.join(root_folder, 'data_robot/gripper_models/panda_gripper/finger.stl')
        ...
        # coords_path = os.path.join(root_folder, 'data_robot/gripper_control_points/panda_gripper_coords.npy')
        with open(os.path.join(root_folder,'data_robot/gripper_control_points/panda_gripper_coords.pickle'), 'rb') as f:
    ...
    def get_control_point_tensor(self, batch_size, use_torch=True, symmetric = False, convex_hull=True):
        ...
        Keyword Arguments:
            use_torch {bool} -- outputing a torch tensor instead of a numpy array (default: {True})
        ...
        control_points = np.load(os.path.join(self.root_folder, 'data_robot/gripper_control_points/panda.npy'))[:, :3]
        if symmetric:
